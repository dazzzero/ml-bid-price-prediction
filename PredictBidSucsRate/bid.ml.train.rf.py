# -*- coding: utf-8 -*-
"""
입찰 성공률 예측 시스템 - 랜덤포레스트 모델 훈련 스크립트
...
- 3개의 RandomForestRegressor 모델을 훈련 (업체투찰률, 예가투찰률, 참여업체수 예측)
...
# 머신러닝 모델과 전처리 도구들
from sklearn.ensemble import RandomForestRegressor  # 랜덤 포레스트 회귀 모델 (여러 의사결정나무의 앙상블)
...
class BidLowerMarginRateTrain():
    ...
    3. 3개의 RandomForestRegressor 모델을 훈련
       - 모델1: 업체 투찰률 예측
       - 모델2: 예가 투찰률 예측  
       - 모델3: 참여 업체 수 예측
    ...
    def generateExcelFileName(self, data_size, test_ratio):
        ...
        # 파일명 생성
        filename = f"result-rf-{time_str}-{data_unit}-{ratio_str}.xlsx"
    ...
    def loadTrainsetFromFile(self, filename):
        ...
        # ===== 학습된 도구들을 파일로 저장 =====
        self.tokenizer.save("rf.tokenizer.v0.1.1.npz")    # 형태소 분석기 저장
        self.vectorizer.save("rf.vectorizer.v0.1.1.npz")  # TF-IDF 벡터화기 저장
    ...
    def setupModels(self):
        """
        3개의 RandomForestRegressor 모델을 설정하는 함수
        ...
        RandomForestRegressor 파라미터 설명:
        - n_estimators: 의사결정나무 개수 (더 많을수록 정확하지만 느림)
        - max_depth: 나무의 최대 깊이 (None이면 제한 없음)
        - min_samples_split: 노드 분할에 필요한 최소 샘플 수
        - min_samples_leaf: 리프 노드의 최소 샘플 수
        - max_features: 각 분할에서 고려할 특성 수 ('sqrt', 'log2', None 등)
        - random_state: 랜덤 시드 (재현 가능한 결과)
        - n_jobs: 병렬 처리할 작업자 수 (-1은 모든 CPU 사용)
        """
        print("="*80)
        print("랜덤 포레스트 모델 설정 시작...")
        print("="*80)
        ...
        model1 = RandomForestRegressor(
                            n_estimators = 200,        # 의사결정나무 200개
                            max_depth = 20,            # 최대 깊이 20
                            min_samples_split = 5,     # 분할 최소 샘플 수 5
                            min_samples_leaf = 2,      # 리프 최소 샘플 수 2
                            max_features = 'sqrt',     # sqrt(특성수)만큼 특성 사용
        ...
        # 모델3: 참여 업체 수 예측 모델 (다른 구조 - 더 많은 나무 사용)
        model3 = RandomForestRegressor(
                            n_estimators = 300,        # 의사결정나무 300개 (더 많음)
                            max_depth = 25,            # 최대 깊이 25 (더 깊음)
                            min_samples_split = 3,     # 분할 최소 샘플 수 3 (더 세밀)
                            min_samples_leaf = 1,      # 리프 최소 샘플 수 1 (더 세밀)
                            max_features = 'log2',     # log2(특성수)만큼 특성 사용
                            random_state = 1,          
                            n_jobs = -1,               
                            verbose = 1                
                            )    
    ...
    def trainnng(self, model, x_trainset, y_trainset):
        ...
        Args:
            model: 훈련시킬 RandomForestRegressor 모델
            x_trainset (list): 훈련용 입력 데이터
            y_trainset (list): 훈련용 출력 데이터
            
        설명:
        - 모델이 입력 데이터를 보고 출력 데이터를 예측하도록 학습
        - 랜덤 포레스트가 여러 의사결정나무를 조합하여 예측 정확도를 높임
        """
        # 주석처리된 디버깅 출력들
        #RandomForestRegressor 클래스의 인스턴스를 생성합니다
        #print("-"*80)
        #print("RandomForestRegressor 모델로 학습을 시작하였습니다. ")
        ...
        print("-"*80)
        print("RandomForestRegressor 모델로 학습을 완료하였습니다. ")
    ...
    def saveModel(self, model, filename):
        ...
        """
        joblib.dump(model, self.save_dir+filename)  # 모델을 파일로 저장 (예: 'rf.model1.v0.0.1.npz')
        
        print("-"*80)
        print("RandomForestRegressor 모델을 저장하였습니다. ")        
...
def Main():
    ...
    # ===== 2단계: 데이터 로드 및 전처리 =====
    x_train, x_test, y_train, y_test = trainer.loadTrainsetFromFile('bid_250921_1.csv')  # CSV 파일에서 데이터 로드
    x_trainset, x_testset = trainer.preprocessingXset(x_train, x_test, 'rf_scaler.v2.npz')  # 입력 데이터 정규화
    ...
    for i, model in enumerate(models):
        ...
        trainer.saveModel(model, f'rf.model{i+1}.v0.1.1.npz')  # 모델 저장
