...
- 3개의 CatBoostRegressor 모델을 훈련 (업체투찰률, 예가투찰률, 참여업체수 예측)
...
# 머신러닝 모델과 전처리 도구들
from catboost import CatBoostRegressor  # CatBoost 회귀 모델
...
class BidLowerMarginRateTrain():
    ...
    3. 3개의 CatBoostRegressor 모델을 훈련
       - 모델1: 업체 투찰률 예측
       - 모델2: 예가 투찰률 예측  
       - 모델3: 참여 업체 수 예측
    ...
    def loadTrainsetFromFile(self, filename):
        ...
        # ===== 학습된 도구들을 파일로 저장 =====
        self.tokenizer.save("cb.tokenizer.v0.1.1.npz")    # 형태소 분석기 저장
        self.vectorizer.save("cb.vectorizer.v0.1.1.npz")  # TF-IDF 벡터화기 저장
    ...
    def setupModels(self):
        """
        3개의 CatBoostRegressor 모델을 설정하는 함수
        ...
        - loss_function: 회귀 손실 (RMSE 등)
        - random_seed: 랜덤 시드
        - verbose: 로그 출력 간격
        """
        print("="*80)
        print("CatBoost 모델 설정 시작...")
        print("="*80)
        
        self.t0 = time()  # 시작 시간 기록

        # ===== 3개의 CatBoost 회귀 모델 설정 =====
        # 참고: CatBoost는 카테고리형 변수 처리가 강점이지만, 여기서는 수치형 입력을 사용
        common_params = {
            'loss_function': 'RMSE',
            'depth': 8,
            'learning_rate': 0.05,
        ...
        # 모델1: 업체 투찰률 예측 모델
        model1 = CatBoostRegressor(iterations=2000, **common_params)

        # 모델2: 예가 투찰률 예측 모델
        model2 = CatBoostRegressor(iterations=2000, **common_params)

        # 모델3: 참여 업체 수 예측 모델 (약간 다른 하이퍼파라미터)
        model3 = CatBoostRegressor(iterations=2500, depth=9, learning_rate=0.04, **{k: v for k, v in common_params.items() if k not in ['depth','learning_rate']})    
    ...
    def trainnng(self, model, x_trainset, y_trainset):
        ...
        print("-"*80)
        print("CatBoostRegressor 모델로 학습을 완료하였습니다. ")
    ...
    def saveModel(self, model, filename):
        ...
        """
        # CatBoost는 자체 저장 포맷(.cbm)을 권장하지만, 일관성을 위해 joblib도 병행 가능
        # 여기서는 .cbm으로 저장
        model.save_model(self.save_dir+filename)
        
        print("-"*80)
        print("CatBoostRegressor 모델을 저장하였습니다. ")        
    ...
    def predict(self, model, x_testset):
        ...
        print("CatBoostRegressor로 테스트셋 예측완료. ")
...
def Main():
    ...
    x_trainset, x_testset = trainer.preprocessingXset(x_train, x_test, 'cb_scaler.v2.npz')  # 입력 데이터 정규화
    ...
    for i, model in enumerate(models):
        ...
        trainer.saveModel(model, f'cb.model{i+1}.v0.1.1.cbm')  # 모델 저장 (CatBoost .cbm)
